      ...
      records
        ...
        .forEach(function (rr) {
          if (rr.type === 'SRV') {
            ...
            // dns-packet@5 may decode TXT to an array of chunks.
            // Convert to a wire-format buffer so dns-txt can decode it.
            if (Array.isArray(txtData)) {
              txtData = Buffer.concat(
                txtData.map(function (chunk) {
                  const tmp = Buffer.isBuffer(chunk)
                    ? chunk
                    : Buffer.from(chunk);
                  const buf = new Uint8Array(tmp.length + 2);
                  const dv = new DataView(buf.buffer);
                  dv.setInt16(0, tmp.length);
                  tmp.copy(buf, 2);
                  return buf;
                }),
              );
